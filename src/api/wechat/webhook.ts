...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    switch (message.MsgType) {
      case 'text':
        processedData = await processTextMessage(message);
        break;
      case 'event':
    ...
    if (processedData && processedData.confidence >= 70) {
      await autoImportDiver(processedData);
    }
...
function processTextMessage(message: WeChatMessage): ParsedDiverData {
  const content = message.Content || '';
  const data: ParsedDiverData = {
    confidence: 0,
    source: 'wechat',
...
async function processEventMessage(message: WeChatMessage): Promise<ParsedDiverData | null> {
...
async function autoImportDiver(data: ParsedDiverData) {
  try {
    // Check for duplicates
    const existingDivers = await apiClient.divers.list();
    const duplicate = existingDivers.find(diver => 
      (data.email && diver.email === data.email) ||
      (data.phone && diver.phone === data.phone)
    );
...
function generateReplyMessage(originalMessage: WeChatMessage, processedData: ParsedDiverData | null): string {
...
export async function setupWeChatMenu() {
  const menu = {
    button: [
      {
        type: "click",
